...
def minimax_ab(board: Board, alpha: int, beta: int, maximazing_player: bool,
               depth: int) -> int:
    """
        Alpha-beta pruning algorithm returning the evaluation
        value through the depth.
    """
    if depth == 0:
    ...
    if maximazing_player == Board.BLUE:
        ...
        for move in board.free_cells():
            board.push(move)
            best_move = max(best_move, minimax_ab(board, alpha, beta,
                            Board.RED, depth - 1))
            board.pop()
        ...
        for move in board.free_cells():
            board.push(move)
            best_move = min(best_move, minimax_ab(board, alpha, beta,
                            Board.BLUE, depth - 1))
            board.pop()
