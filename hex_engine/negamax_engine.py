...
def negamax_ab(board: Board, alpha: int, beta: int, maximazing_player: bool, depth: int) -> int:
	"""
	...
	for move in board.free_cells():
		board.push(move)
		best_move = max(best_move, -negamax_ab(board, -beta, -alpha, not maximazing_player, depth - 1))
		board.pop()
